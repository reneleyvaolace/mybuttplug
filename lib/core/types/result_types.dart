...
///   (device) => print('Conectado: ${device.name}'),
/// );
/// ```
abstract class Result<T, E> {
  ...
  T get value {
    return fold(
      (error) => throw ResultException('Attempted to get value from Failure'),
      (value) => value,
    );
  }
  ...
  E get error {
    return fold(
      (error) => error,
      (value) => throw ResultException('Attempted to get error from Success'),
    );
  }
  ...
  Future<Result<U, E>> asyncMap<U>(Future<U> Function(T value) f) async {
    return fold<Result<U, E>>(
      (error) => Failure(error),
      (value) async => await f(value).then(Success.new),
    );
  }
  
  /// Mapear asíncrono el error
  Future<Result<T, U>> asyncMapError<U>(Future<U> Function(E error) f) async {
    return fold<Result<T, U>>(
      (error) async => await f(error).then(Failure.new),
      (value) => Success(value),
    );
  }
