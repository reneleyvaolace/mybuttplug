...
class ProtocolAdapter {
  ...
  Result<SpecificCommand, ProtocolError> translate(
    ...
    if (protocol == null) {
      return Failure(ProtocolError.unknownProtocol);
    }
  ...
  Future<Result<void, DeviceError>> translateAndSend(
    ...
    return translationResult.fold(
      (error) => Failure(DeviceError.unsupportedProtocol),
...
class SpecificCommand {
  ...
  List<int> toBytes() {
    ...
    if (textCommand != null) return textCommand!.codeUnits;
    throw StateError('No hay bytes ni comando de texto');
  }
  ...
  String toString() {
    if (bytes != null) {
      ...
      return 'SpecificCommand($protocolName: $textCommand)';
    }
    return 'SpecificCommand($protocolName)';
  }
}
