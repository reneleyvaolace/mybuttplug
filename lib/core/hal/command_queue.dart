...
class CommandQueue {
  ...
  Future<Result<void, CommandError>> enqueueSequence(
    ...
    if (hasErrors) {
      return Failure(CommandError.executionFailed);
    }
  ...
  Future<void> _processQueue() async {
    ...
    while (_queue.isNotEmpty) {
      ...
      _eventController.add(CommandExecutingEvent(
      ...
      try {
        ...
        final result = await _executeWithTimeout(queued).timeout(
          queued.timeout,
          onTimeout: () => Failure(CommandError.timeout),
        );
        
        queued.completer.complete(result);
        
        _eventController.add(CommandCompletedEvent(
          commandId: queued.command.commandId,
          success: result.isSuccess,
        ));
      } catch (e) {
        queued.completer.complete(Failure(CommandError.executionFailed));
        
        _eventController.add(CommandFailedEvent(
          commandId: queued.command.commandId,
          error: e.toString(),
        ));
      }
  ...
  Future<Result<void, CommandError>> _executeWithTimeout(
    ...
    while (attempts <= maxRetries) {
      try {
        // Ejecutar comando
        await _executeCommand(queued.command);
        return Success(null);
      } catch (e) {
        attempts++;
    ...
    return Failure(CommandError.maxRetriesExceeded);
  }
...
/// Comando encolado
class CommandQueuedEvent extends QueueEvent {
...
class CommandExecutingEvent extends QueueEvent {
  final String commandId;
  final int queueLength;
...
/// Comando completado
class CommandCompletedEvent extends QueueEvent {
  final String commandId;
  final bool success;
...
/// Comando fallido
class CommandFailedEvent extends QueueEvent {
  final String commandId;
  final String error;
...
var _executeCommand = (GenericCommand command) async {};
